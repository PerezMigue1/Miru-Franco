...
/**
 * Duplicado de admin/clientes-crm/[id] para operación (estilista/empleado/becario con
 * `clientes:lectura`). No comparte componente porque las acciones disponibles difieren
 * (sin editar/eliminar, que son admin-only) — ver TAREA "Detalle de cliente en operación".
 *
 * A diferencia de la versión admin, la identidad base del cliente viene de
 * GET /api/clientes/:id (gateado por `clientes:lectura`) en vez de GET /api/usuarios/:id
 * (gateado por OwnerOrAdminGuard, admin-only) — por eso Rol, Google ID y Acepta aviso
 * privacidad no están disponibles aquí (no vienen en ese payload y no son necesarios
 * para el trabajo diario del staff).
 */
export default function ClienteDetalleOperacionPage() {
      ...
      <OperacionLayout permisoRequerido="clientes:lectura">
        <div className="w-full max-w-none py-12">
          ...
          <div className="text-center mt-4">
            <Button variant="outline" onClick={() => router.push('/operacion/segmentacion-clientes')}>
              Volver al listado
            </Button>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              <Card variant="elevated" padding="lg">
                ...
                <div className="space-y-2">
                  ...
                  <Button fullWidth variant="outline" onClick={() => router.push('/operacion/segmentacion-clientes')}>
                    Volver al listado
                  </Button>
                </div>
              </Card>
