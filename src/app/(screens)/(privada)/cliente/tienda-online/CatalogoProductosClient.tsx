...
export default function CatalogoProductosClient({ initialProductos }: Props) {
  ...
  const paginaEfectiva = resetPaginaPorFiltro ? 1 : Math.min(Math.max(1, paginaActual), totalPaginas);
  ...
  const rangoDesde = productosOrdenados.length === 0 ? 0 : (paginaEfectiva - 1) * PRODUCTOS_POR_PAGINA + 1;
      ...
      <div className="mb-6 flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {categorias.map((cat) => {
            ...
            return (
              <Button key={cat} size="sm" variant={isActive ? 'chip' : 'outline'} onClick={() => setCategoriaSeleccionada(cat)}>
                {cat}
              </Button>
            );
          })}
        ...
        <aside className="lg:w-72 flex-shrink-0">
          <Card className="p-4 space-y-6" style={{ backgroundColor: 'var(--tarjetas-paneles)' }}>
            ...
            {mostrarFiltro('precio') && (
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <Input type="number" placeholder="Mín" value={precioMin} onChange={(e) => setPrecioMin(e.target.value)} min={0} className="w-24" />
                  <span style={{ color: 'var(--encabezados-alterno)' }}>–</span>
                  <Input type="number" placeholder="Máx" value={precioMax} onChange={(e) => setPrecioMax(e.target.value)} min={0} className="w-24" />
                </div>
              </div>
            )}
                  ...
                  {OPCIONES_DESCUENTO.map((opt) => (
                    <li key={opt.min}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={descuentos.includes(opt.min)} onChange={() => toggleFiltro(descuentos, setDescuentos, opt.min)} className="rounded" />
                        {opt.label}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 max-h-32 overflow-y-auto">
                  {marcasUnicas.length ? marcasUnicas.map((m) => (
                    <li key={m}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={marcas.includes(m)} onChange={() => toggleFiltro(marcas, setMarcas, m)} className="rounded" />
                        {m}
                      </label>
                    </li>
              ...
              <div>
                ...
                <ul className="space-y-1.5 max-h-40 overflow-y-auto">
                  {OPCIONES_COLOR.map((c) => (
                    <li key={c}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={colores.includes(c)} onChange={() => toggleFiltro(colores, setColores, c)} className="rounded" />
                        {c}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 max-h-32 overflow-y-auto">
                  {OPCIONES_ACABADO.map((a) => (
                    <li key={a}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={acabados.includes(a)} onChange={() => toggleFiltro(acabados, setAcabados, a)} className="rounded" />
                        {a}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {OPCIONES_FORMATO_LABIAL.map((f) => (
                    <li key={f}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={formatosLabial.includes(f)} onChange={() => toggleFiltro(formatosLabial, setFormatosLabial, f)} className="rounded" />
                        {f}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 flex flex-wrap gap-x-3 gap-y-1">
                  {OPCIONES_UNIDADES_PACK.map((u) => (
                    <li key={u}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={unidadesPack.includes(u)} onChange={() => toggleFiltro(unidadesPack, setUnidadesPack, u)} className="rounded" />
                        {u}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 max-h-32 overflow-y-auto">
                  {OPCIONES_TIPO_PIEL.map((t) => (
                    <li key={t}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={tipoPiel.includes(t)} onChange={() => toggleFiltro(tipoPiel, setTipoPiel, t)} className="rounded" />
                        {t}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {OPCIONES_TEXTURA.map((t) => (
                    <li key={t}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={textura.includes(t)} onChange={() => toggleFiltro(textura, setTextura, t)} className="rounded" />
                        {t}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {OPCIONES_SPF.map((s) => (
                    <li key={s}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={spf.includes(s)} onChange={() => toggleFiltro(spf, setSpf, s)} className="rounded" />
                        {s}
                      </label>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 max-h-32 overflow-y-auto">
                  {caracteristicasUnicas.length ? caracteristicasUnicas.map((c) => (
                    <li key={c}>
                      <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--menu-texto-principal)' }}>
                        <input type="checkbox" checked={otrasCaracteristicas.includes(c)} onChange={() => toggleFiltro(otrasCaracteristicas, setOtrasCaracteristicas, c)} className="rounded" />
                        {c}
                      </label>
                    </li>
            ...
            <Button variant="outline" size="sm" fullWidth onClick={() => {
              setColores([]); setDescuentos([]); setPrecioMin(''); setPrecioMax('');
              setMarcas([]); setAcabados([]); setFormatosLabial([]); setUnidadesPack([]);
              setTipoPiel([]); setTextura([]); setSpf([]); setOtrasCaracteristicas([]);
            }}>
              Limpiar filtros
            </Button>
          </Card>
        ...
        <div className="flex-1 min-w-0">
          {loading && (
            <Card className="text-center py-12">
              <p className="text-lead" style={{ color: 'var(--encabezados-alterno)' }}>Cargando productos...</p>
            </Card>
          )}
          {error && (
            <Card className="text-center py-12 max-w-xl mx-auto" style={{ borderColor: 'var(--danger)' }}>
              <p className="text-lead mb-4" style={{ color: 'var(--danger)' }}>{error}</p>
              {error.includes('conectar') && (
                <div className="text-sm mb-4 text-left" style={{ color: 'var(--encabezados-alterno)' }}>
                  ...
                  <code className="block mb-3 p-2 rounded break-all bg-black/10">{errorUrl ?? '…'}</code>
                </div>
              )}
          ...
          {!loading && !error && (
            <div id="catalogo-productos-grid" className="space-y-6 scroll-mt-4">
                ...
                {productosPagina.map((producto) => {
                  ...
                  return (
                    ...
                    >
                      {noDisponible && (
                        <>
                          <div className="absolute inset-0 z-10 rounded-lg pointer-events-none" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} />
                          <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none select-none" style={{ color: 'rgba(255,255,255,0.95)', fontSize: 'clamp(4rem, 18vw, 9rem)', fontWeight: 800, textShadow: '0 0 30px rgba(0,0,0,0.9)' }}>✕</div>
                        </>
                      )}
                      <div className="relative mb-4">
                        <div className="relative mb-4 h-64 w-full overflow-hidden rounded-lg" style={{ backgroundColor: 'var(--fondos-suaves)' }}>
                          <div className="relative z-0 h-full min-h-[16rem] w-full">
                            <ProductoImagenCarruselTarjeta urls={urlsGaleriaProductoCatalogo(producto)} alt={producto.nombre} />
                          </div>
                      ...
                      <div className="mb-2"><Badge variant="info" size="sm">{producto.categoria || 'Producto'}</Badge></div>
                      <h3 className="text-subtitle mb-2" style={{ color: 'var(--menu-texto-principal)' }}>{producto.nombre}</h3>
                      <p className="text-sm mb-3" style={{ color: 'var(--encabezados-alterno)' }}>{producto.descripcion}</p>
                      <div className="flex items-center justify-between">
                        <div>
                          {producto.precioOriginal && <p className="text-sm line-through" style={{ color: 'var(--encabezados-alterno)' }}>{producto.precioOriginal}</p>}
                          <p className="text-xl font-bold" style={{ color: 'var(--menu-texto-principal)' }}>{producto.precio}</p>
                        </div>
                        <Button size="sm" onClick={(e) => { e.stopPropagation(); abrirDetalleProducto(producto.id); }}>Ver Detalles</Button>
                      </div>
                    </Card>
                  );
                })}
              ...
              {productosOrdenados.length > 0 && (
                <div className="flex flex-col sm:flex-row flex-wrap items-center justify-between gap-3 pt-2 border-t" style={{ borderColor: 'var(--fondos-suaves)' }}>
                  <p className="text-sm" style={{ color: 'var(--encabezados-alterno)' }}>
                    Mostrando {rangoDesde}–{rangoHasta} de {productosOrdenados.length} productos
                  </p>
                  <div className="flex items-center gap-2">
                    <Button type="button" size="sm" variant="outline" disabled={paginaEfectiva <= 1} onClick={() => irAPagina(paginaEfectiva - 1)}>Anterior</Button>
                    <span className="text-sm px-2" style={{ color: 'var(--menu-texto-principal)' }}>Página {paginaActual} de {totalPaginas}</span>
                    <Button type="button" size="sm" variant="outline" disabled={paginaEfectiva >= totalPaginas} onClick={() => irAPagina(paginaEfectiva + 1)}>Siguiente</Button>
                  </div>
                </div>
              )}
            </div>
          ...
          {!loading && !error && productosFiltrados.length === 0 && (
            <Card className="text-center py-12">
              <p className="text-lead" style={{ color: 'var(--encabezados-alterno)' }}>No se encontraron productos con los filtros seleccionados</p>
            </Card>
          )}
        </div>
      </div>
